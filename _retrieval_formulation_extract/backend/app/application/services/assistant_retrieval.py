...
def _proper_noun(question: str, norm: str) -> str | None:
    ...
    - sentence-initial tokens UNLESS the question is a document reference
      (it names a year or a file extension like ``.pdf``): ``Cblu Jan
      2024`` must resolve to ``cblu``, but Hinglish ``Maine CBLU me ...``
      ("I ...") must never resolve to ``maine``.
    """
    tokens = (question or "").split()
    has_doc_ref = _YEAR_RE.search(norm) is not None or any(
        token.strip("'\".,!?;:’‘").lower().endswith(_DOC_REF_EXTENSIONS)
        for token in tokens
    )
    for idx, token in enumerate(tokens):
        ...
        if lowered in _QUERY_STOPWORDS:
            continue
        if idx == 0 and not has_doc_ref:
            # Sentence-initial capitalized word without a year/extension
            # signal is usually the question word or an imperative — not a
            # trustworthy entity.
            continue
        return lowered
    return None
