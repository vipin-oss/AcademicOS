...
class TestChunkEvidencePath:
    ...
    def test_retrieval_finds_document(self, harness):
        """Backend-agnostic: the document is found via whatever retrieval
        backend is wired (LIKE content leg on bd253b9, FTS on P1) — the
        chunk-evidence path is what this test pins."""
        repo = SQLAlchemySearchRepository(harness["session"])
        hits = repo.search(text="ramanujan", limit=8)
