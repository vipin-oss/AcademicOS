...
def make_scanned_pdf_bytes() -> bytes:
    ...
    Builds a minimal PDF whose content stream draws an image placeholder; the
    extracted text is empty so the parser flags ``needs_ocr``.
    """
    # Reuse the hand-written PDF builder but with empty text -> empty text layer.
