# AcademicOS — graceful Windows shutdown (Sprint M10.1)
...
if ($backend) {
    ...
    Write-OK "Backend stopped (PID $pidToStop)"
} else {
...
if ($frontend) {
    ...
    Stop-Process -Id $pidToStop -Force -ErrorAction SilentlyContinue
    Write-OK "Frontend stopped (PID $pidToStop)"
} else {
...
if (-not $KeepQdrant) {
    ...
    if ($hasDocker) {
        $name = docker ps --format "{{.Names}}" | Select-String -Quiet "academicos-qdrant"
        if ($name) {
